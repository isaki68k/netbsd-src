...
void
audio_pmixer_intr(struct audio_softc *sc)
{
	audio_trackmixer_t *mixer;
...
audio_rmixer_process(struct audio_softc *sc)
{
	...
	SLIST_FOREACH(f, &sc->sc_files, entry) {
		...
		TRACE(track, "broadcast; inp=%d/%d/%d",
		    input->top, input->count, input->capacity);
	}
...
// sc_intr_lock で呼び出されます。
void
audio_rmixer_intr(struct audio_softc *sc)
{
	audio_trackmixer_t *mixer;
	...
	audio_file_t *file)
{
	...
	while (uio->uio_resid > 0 && error == 0) {
		...
		TRACE(track, "while resid=%zd input=%d/%d/%d usrbuf=%d/%d/%d",
		    uio->uio_resid,
